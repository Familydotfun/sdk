...
export type ModulePermission =
  | "identity:read"           // ANS name, avatar, role in family
  | "identity:wallet"         // connected wallet address
  | "identity:balance"        // user token balance in this family
  | "house:read"              // name, symbol, member count
  | "members:read"            // member list + roles
  | "social:post"             // post to Town Hall / feed / thesis / polls
  | "social:comment"          // comment on posts
  | "social:react"            // react / vote on posts
  | "events:read"             // read campaigns/events
  | "events:write"            // create campaigns/events
  | "content:read"            // read wiki and long-form content
  | "content:write"           // create / edit wiki and long-form content
  | "treasury:read"           // treasury balances
  | "treasury:deposit"        // receive funds into treasury
  | "treasury:spend:limited"  // spend up to a cap (time-locked)
  | "treasury:spend:propose"  // create spend proposals only
  | "payments:tip"            // user-initiated tips to members
  | "payments:escrow"         // deposit/withdraw user funds to app-controlled contracts
  | "contract:invoke"         // call declared methods on app-controlled contracts
  | "wiki:read"               // read family wiki pages
  | "wiki:write"              // create / edit wiki pages
  | "polls:read"              // read polls
  | "polls:write"             // create polls and vote
  | "campaigns:read"          // read campaigns / quests
  | "campaigns:write"         // create and manage campaigns
  | "thesis:read"             // read family theses
  | "thesis:write"            // create and react to theses
  | "store:read"              // read this module's scoped key-value storage
  | "store:write"             // write this module's scoped key-value storage
  // Read-only surface scopes. Public data — no user grant required.
  | "profile:read"            // read family profile info and stats
  | "chart:read"              // read prices, OHLCV bars and trades
  | "vault:read"              // read vault holdings and activity
  | "governance:read"         // read governance proposals
  // User-app scopes (install-to-user-account apps).
  | "user:identity"           // wallet + profile of the current user
  | "user:balance"            // read the user's in-app balance
  | "payments:charge"         // platform-mediated charges with enforced split
  | "storage:upload"          // host-bridged file/image uploads
  | "trading:agent";          // app holds a trade-only agent key (never withdraws)

export type ModuleRiskLevel = "low" | "medium" | "high" | "critical";

export type ModuleCommandArgType =
  | "user"
  | "number"
  | "symbol"
  | "address"
...
export interface ModuleCommandSpec {
  ...
  description: string;
  usage: string;
  args: { name: string; type: ModuleCommandArgType }[];
...
export interface HouseMember {
  wallet: string;
  name: string | null; // ANS primary name
...
export function isFamilyContext(ctx: SDKContext): ctx is FamilyContext {
  return ctx.mode === "family";
}

export function isUserContext(ctx: SDKContext): ctx is UserContext {
...
export interface HouseIdentity {
  ...
  avatarUrl?: string | null;
  role: "head" | "moderator" | "holder" | "system";
  balance: string; // token balance in this family
}
...
export interface ModuleManifest {
  ...
  entryUrl?: string; // for external iframe apps
}

export interface ModuleDefinition extends ModuleManifest {
  source: "internal" | "external";
  component?: string; // internal component name
  riskScore: number; // 0-100 computed from permissions
}
...
export interface FamilySDK {
  ...
  social: {
    ...
    votePoll: (postId: string, optionId: string) => Promise<{ ok: boolean; error?: string }>;
  };
  wiki: {
    ...
    deletePage: (slug: string) => Promise<{ ok: boolean; error?: string }>;
  };
  campaigns: {
    ...
    approve: (slug: string, vote: "yes" | "no") => Promise<{ ok: boolean; error?: string }>;
    submit: (slug: string, contentUrl: string, caption?: string, platform?: string) => Promise<{ ok: boolean; submission?: unknown; error?: string }>;
    voteSubmission: (slug: string, submissionId: string, score: number) => Promise<{ ok: boolean; error?: string }>;
    finalizeApproval: (slug: string) => Promise<{ ok: boolean; approved?: boolean; error?: string }>;
    close: (slug: string) => Promise<{ ok: boolean; error?: string }>;
  };
  thesis: {
    list: () => Promise<ThesisListResult>;
    create: (input: ThesisInput) => Promise<ThesisResult>;
    react: (postId: string, stickerId: string) => Promise<{ ok: boolean; removed?: boolean; error?: string }>;
  };
  products: {
  ...
  contract: {
    invoke: (
      ...
      method: string,
      args: unknown[],
      options?: { value?: string; abi?: unknown }
  ...
  store: {
    ...
    set: (key: string, value: unknown) => Promise<{ ok: boolean; key: string }>;
    delete: (key: string) => Promise<{ ok: boolean; deleted: number }>;
...
export type HouseAppMessageType =
  | "FAMILY:INIT"
  | "FAMILY:INIT:RESPONSE"
  | "FAMILY:CALL"
